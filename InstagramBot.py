...
class InstaBot:
    ...
    def unfollow_people(self, names):
        ...
        self.following_number = int(self.browser.find_element_by_xpath('//*[@id="react-root"]/section/main/div/header/section/ul/li[3]/a/span').text)
        self.scroll_window = self.browser.find_element_by_xpath("//div[@role='dialog']//a")
        counter = 0
        while counter < int(self.following_number / 5):
            self.scroll_window = self.browser.find_element_by_xpath("//div[@role='dialog']//a")
            self.scroll_window.send_keys(Keys.PAGE_DOWN)
            counter += 1
            time.sleep(0.5)
        self.unfollow_buttons = self.browser.find_elements_by_xpath("/html/body/div[4]/div/div/div[2]/ul/div/li[2]/div")
